...
http.createServer(async function(req, res) {
    ...
    if (req.method == 'PUT') {
        ...
        if (parts.length > 1) {
            ...
            connect(con)
            .then(async () => {
                await deleteQuestion(con, parts[1])
                .then((qResults) => {
                    deleteOptions(con, parts[1])
                    .then((opResults) => {
                        end(con);
                        res.end(JSON.stringify({
                            worked: qResults.affectedRows == 1,
                            questionResults: qResults,
                            optionResults: opResults
                        }));
                    });
                });
            });       
...
function deleteQuestion(con, question_id) {
...
function deleteOptions(con, question_id) {

    return new Promise((resolve, reject) => {

        let sql = " DELETE FROM _options WHERE question_id = ? ";
